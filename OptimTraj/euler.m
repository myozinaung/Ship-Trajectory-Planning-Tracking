...
%
%   problem.options.method = 'euler'
%   problem.options.euler = struct with method parameters:
...
nSegment = Opt.euler.nSegment;
nSubStep = Opt.euler.nSubStep;
nGridControl = 2*nSegment*nSubStep + 1;
nGridState = nSegment + 1;

% Print out some solver info if desired:
if Opt.verbose > 0
    fprintf('  -> Transcription via Euler Shooting method \n');
    fprintf('        nSegments = %d \n', nSegment);
    fprintf('        nSubSteps = %d \n', nSubStep);
end
...
if strcmp(Opt.euler.adaptiveDerivativeCheck,'on')
    if exist('jacobianest','file')
        ...
        Opt.euler.adaptiveDerivativeCheck = 'cannot find jacobianest.m';
...
soln.interp.control = @(t)( interp1(tSoln(1:2:end)', uSoln(:,1:2:end)', t','pchip')' );
...
%
global EULER_t EULER_x EULER_u
global EULER_defects EULER_pathCost
global EULER_decVars
%
usePreviousValues = false;
if ~isempty(EULER_decVars)
    if length(EULER_decVars) == length(decVars)
        if ~any(EULER_decVars ~= decVars)
...
if usePreviousValues
    ...
    x = EULER_x;
    u = EULER_u;
    defects = EULER_defects;
    pathCost = EULER_pathCost;
else
    %
    %
...
%         k3 = combinedDynamics(t0+dt,     x0 +     dt*k2(1:nState,:), u(:,idx+1), dynFun,pathObj);
        z = dt*k0;  %Change over the sub-step
    ...
    EULER_x = x;
    EULER_u = u;
    EULER_defects = defects;
    EULER_pathCost = pathCost;
...
global EULER_t EULER_x EULER_u
global EULER_defects EULER_pathCost
global EULER_decVars EULER_dxdalpha EULER_dJdalpha
%
usePreviousValues = false;
if ~isempty(EULER_decVars)
    if length(EULER_decVars) == length(decVars)
...
if usePreviousValues
    ...
    defects = EULER_defects;
    pathCost = EULER_pathCost;
    dxdalpha = EULER_dxdalpha;
    dJdalpha = EULER_dJdalpha;
else
    %
    %
    %%%% END CODE OPTIMIZATION %%%%
...
%             dk3da = dk3(:,:,j) * [dt_dalpha + 1/(nTime-1)*dTdalpha; dxdalpha{j}(:,:,iSubStep) + dt*dk2da(1:nState,:) + 1/(nTime-1)*k2(1:nState,j)*dTdalpha; du1_dalpha];
    ...
    EULER_defects = defects;
    EULER_pathCost = pathCost;
    EULER_dxdalpha = dxdalpha;
    EULER_dJdalpha = dJdalpha;

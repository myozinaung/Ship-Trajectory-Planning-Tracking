...
% knitro_nlp_options = knitro_options(...
% 'algorithm', 0, ... % 0(auto), 1(IP), 4(sqp)
% 'outlev', 4 ,...
% 'maxit', 1000, ...
% 'xtol', 1e-5, ...
% 'feastol', 1e-5, ...
...
tic;
[zSoln, objVal,exitFlag,output] = fmincon(P); % Original
% [zSoln, objVal,exitFlag,output] = fmincon(P.objective,P.x0,[],[],[],[],P.lb, P.ub, P.nonlcon,P.options); % test by MYO
% [zSoln, objVal,exitFlag,output] = fmincon_myo(P.objective,P.x0,P.lb, P.ub, P.nonlcon,P.options); % test by MYO

% [zSoln, objVal,exitFlag,output] = knitro_nlp(P.objective,P.x0,[],[],[],[],P.lb, P.ub, P.nonlcon,[],knitro_nlp_options); % test by MYO
% Knitro observation --> much slower than fmincon, but has automatic algorithm selection

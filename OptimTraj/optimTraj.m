...
while iter <= nIter
    ...
    % Not to use infeasible solution as a guess for next mesh refinement
%      if iter == 3 && soln(2).info.exitFlag == -2 && pass == 1 % if infeasible, do iter 2 again with original guess (MYO)
%          iter = 2;
%          P.options = problem.options(iter);
%          P.guess   = problem.guess;
%          pass = 0; % to do again only one time, if still infeasible again, just go ahead
%          disp('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~');
%          disp('Recalculating iteration 2 with Original Initial Guess');
%      end  
